...
const app = {
    ...
    initData: function (alertMsg = true) {
        if (alertMsg && !confirm("This will replace all current data with random examples. Continue?")) return;
    ...
    handleFileUpload: function (input) {
        ...
        reader.onload = (e) => {
            ...
            // Ask user mode
            if (confirm("Click OK to REPLACE all data, or Cancel to APPEND to existing data?")) {
                this.processCSV(text, true); // Replace
                this.showToast('Data Replaced Successfully', 'success');
            } else {
                this.processCSV(text, false); // Append
                this.showToast('Data Appended Successfully', 'success');
            }
    ...
    processCSV: function (csvText, replace) {
        ...
        let start = 0;
        if (lines[0] && lines[0].includes('지사명')) start = 1;
        if (lines[0] && lines[0].includes('Branch')) start = 1;
        ...
        for (let i = start; i < lines.length; i++) {
            ...
            newData.push({
                ...
                // If CSV has value (idx 6+), use it. Else random (if init) or 0 (if upload)
    ...
    fillBranchSelect: function (id) {
        ...
        if (el) el.innerHTML = `<option value="">Select Branch</option>` + brs.map(b => `<option>${b}</option>`).join('');
    },
    login: function () {
        ...
        if (document.getElementById('formAdmin').style.display === 'block') {
            if (adId === 'admin') u = { role: 'ADMIN', name: 'Administrator', branch: 'HQ' };
        } else if (document.getElementById('formBranch').style.display === 'block') {
            if (br) u = { role: 'BRANCH', name: br + ' Manager', branch: br };
        } else {
            if (stBr) u = { role: 'STAFF', name: 'Staff User', branch: stBr };
        }
        ...
        if (u) {
            ...
            this.showToast('Check credentials. Admin ID: admin', 'error');
        }
    },
    showApp: function () {
    ...
    renderAll: function () {
        if (!this.user) return;
        this.renderStats();
        // Delay chart rendering slightly to ensure container visibility
    ...
    renderList: function () {
        ...
        if (!list.length) { tbody.innerHTML = `<tr><td colspan="8" style="text-align:center; padding:20px;">No data</td></tr>`; return; }
    ...
    downloadCSV: function () {
        let csv = "Branch,Code,Manager,TargetNew,TargetCancel,Phone,New,Sub,Cancel,Suspend,Retention,Note\n";
        this.data.forEach(d => {
